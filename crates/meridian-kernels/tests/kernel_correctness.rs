...
fn entropy_kernel_matches_cpu_reference() {
    ...
    for &vocab in &[1024usize, 16_000, 32_000, 151_936] {
        for seed in 0..3u64 {
            let host: Vec<f32> =
                make_logits(seed.wrapping_mul(7).wrapping_add(vocab as u64), vocab);
            let dev: CudaSlice<f32> = stream.memcpy_stod(&host).expect("host -> device copy");
            let out_dev: CudaSlice<f32> =
                stream.memcpy_stod(&[0.0_f32]).expect("alloc + zero output");
