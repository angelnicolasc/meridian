...
fn event_to_py(py: Python<'_>, ev: PhaseEvent) -> PyResult<PyObject> {
    ...
    match ev {
        ...
        PhaseEvent::ForceBudget { inject_token, reason } => {
...
impl PyEntropySignal {
    ...
    #[getter]
    fn token_entropy(&self) -> f32 { self.inner.token_entropy }
    #[getter]
    fn eat(&self) -> f32 { self.inner.eat }
    #[getter]
    fn eat_ema(&self) -> f32 { self.inner.eat_ema }
    #[getter]
    fn eat_ema_variance(&self) -> f32 { self.inner.eat_ema_variance }
...
impl PyMeridianScheduler {
    ...
    fn schedule_batch(
        ...
        let batch = self.inner.schedule_batch(output_budget_slots, available_blocks);
        let d = PyDict::new(py);
    ...
    #[getter]
    fn output_queue_depth(&self) -> usize { self.inner.output_queue_depth() }
    #[getter]
    fn think_queue_depth(&self) -> usize { self.inner.think_queue_depth() }
}
...
fn tier_from_str(s: &str) -> PyResult<BlockTier> {
    match s {
        ...
        other => Err(PyValueError::new_err(format!("unknown block tier: {other}"))),
    }
}
...
impl PyBlockManager {
    ...
    fn allocate(&self, req_id: u64, tier: &str, n_blocks: u32) -> PyResult<Vec<u32>> {
        ...
        let mut mgr = self.inner.lock().map_err(|e| PyValueError::new_err(e.to_string()))?;
    ...
    fn used_bytes(&self) -> PyResult<u64> {
        Ok(self.inner.lock().map_err(|e| PyValueError::new_err(e.to_string()))?.used_bytes())
    }

    #[getter]
    fn capacity_bytes(&self) -> PyResult<u64> {
        Ok(self.inner.lock().map_err(|e| PyValueError::new_err(e.to_string()))?.capacity_bytes())
    }

    #[getter]
    fn available_bytes(&self) -> PyResult<u64> {
        let mgr = self.inner.lock().map_err(|e| PyValueError::new_err(e.to_string()))?;
...
fn load_config(py: Python<'_>, path: &str) -> PyResult<PyObject> {
    let cfg = CoreConfig::from_path(path)
        .map_err(|e| PyValueError::new_err(format!("{e}")))?;
